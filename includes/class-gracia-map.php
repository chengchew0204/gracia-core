...
class Gracia_Map {
    ...
    private function enqueue_assets(): void {
        ...
        wp_enqueue_script(
            'gracia-map-pan',
            GRACIA_PLUGIN_URL . 'assets/js/map-pan.js',
            [],
            GRACIA_VERSION,
            true
        );
    ...
    private function apply_js_patches( string $html ): string {
        ...
        // 7. Demo cursor: always use mapscroll polling before starting the demo.
        // camp.mx only polls mapscroll on mobile; on desktop it fires after a
        // blind 400ms timeout.  gracia.la's pan is driven by map-pan.js which
        // sets window.mapscroll = 0 on completion, so forcing the polling path
        // for all devices ensures the demo waits for the pan to finish.
        $search[]  = 'this.isMobileDevice()';
        $replace[] = 'true';
